...
public class SignatureBaseStringTest extends SignpostTestBase {
    ...
    HttpRequest httpGet = httpGET("http://example.com");

    HttpRequest httpPost = httpPOST("http://example.com");

    @Test
    public void shouldConsistOf3NonEmptyPartsConcatenatedWithAmpersand()
            throws Exception {
        SignatureBaseString sbs = new SignatureBaseString(
                httpPOST("http://example.com"), OAUTH_PARAMS);
    ...
    public void shouldStartWithUppercaseHttpMethod() throws Exception {
        assertTrue(new SignatureBaseString(httpPost, OAUTH_PARAMS).compute().split(
                "&")[0].equals("POST"));
    ...
    public void shouldNormalizeRequestUrl() throws Exception {
        ...
        String outputUrl = new SignatureBaseString(httpGet, OAUTH_PARAMS).normalizeUrl(inputUrl);
        ...
        assertFalse(new SignatureBaseString(httpGet, OAUTH_PARAMS).normalizeUrl(
    ...
    public void shouldNormalizeParameters() throws Exception {
        ...
        String result = new SignatureBaseString(httpGet, oauthParams).normalizeParameters(params);
        ...
        result = new SignatureBaseString(httpGet, oauthParams).normalizeParameters(params);
        assertEquals(expected, result);
